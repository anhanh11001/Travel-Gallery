...
public class CustomImageView extends AppCompatImageView implements ScaleGestureDetector.OnScaleGestureListener {
    ...
    public void setActivity(DisplayImageActivity activity) {displayImageActivity = activity;}
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        TypedArray values = context.obtainStyledAttributes(attrs, R.styleable.CustomImageView);

        zoomable = values.getBoolean(R.styleable.CustomImageView_zoomable, true);
        translatable = values.getBoolean(R.styleable.CustomImageView_translatable, true);
        animateOnReset = values.getBoolean(R.styleable.CustomImageView_animateOnReset, true);
        autoCenter = values.getBoolean(R.styleable.CustomImageView_autoCenter, true);
        restrictBounds = values.getBoolean(R.styleable.CustomImageView_restrictBounds, false);
        doubleTapToZoom = values.getBoolean(R.styleable.CustomImageView_doubleTapToZoom, true);
        minScale = values.getFloat(R.styleable.CustomImageView_minScale, MIN_SCALE);
        maxScale = values.getFloat(R.styleable.CustomImageView_maxScale, MAX_SCALE);
        doubleTapToZoomScaleFactor = values.getFloat(R.styleable.CustomImageView_doubleTapToZoomScaleFactor, 3);
        autoResetMode = AutoResetMode.Parser.fromInt(values.getInt(R.styleable.CustomImageView_autoResetMode, AutoResetMode.UNDER));
        ...
        if (!isClickable() && isEnabled() && (zoomable || translatable)) {
            ...
            if (doubleTapToZoom && doubleTapDetected) {
                ...
                if (displayImageActivity != null) {
                    displayImageActivity.toggleActionBar();
                }
            }
